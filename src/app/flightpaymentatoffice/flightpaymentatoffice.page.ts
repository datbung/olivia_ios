          ...
          if (!check && se.allowCheckHoldTicket) {
            ...
            if(check){
              ...
              let itemcache = se._flightService.itemFlightCache;
               //Có chọn khách sạn thì gọi thêm updatepayment theo luồng check ks
              if(se._flightService.itemFlightCache.objHotelCitySelected){
                var url = C.urls.baseUrl.urlContracting + '/build-link-to-pay-aio?paymentType=8&source=app&amount=' + itemcache.totalPrice + '&orderCode=' + (itemcache.pnr.bookingCode ?itemcache.pnr.bookingCode:  itemcache.pnr.resNo) + '&rememberToken=&buyerPhone=' + itemcache.phone;
