        ...
        if( se.returnTimeRange && se.returnTimeRange.length >0){//Lọc theo giờ hạ cánh
          ...
          if( se.classSelected && se.classSelected.length >0){
            let filterclass = listFilter.filter((filterclassitem) => {
              ...
              //   str = str ? (str || filterclassitem.ticketClass == "Linh hoạt") : filterclassitem.ticketClass == "Linh hoạt" ;
              // }
              // if(se.classSelected.indexOf(4) != -1){
              //   str = str ? (str || filterclassitem.ticketClass == "Tiết kiệm" || ((filterclassitem.ticketClass == "Phổ thông đặc biệt") && filterclassitem.ticketCondition && !filterclassitem.ticketCondition.isTicketRefund && !filterclassitem.ticketCondition.isChangeSegment )) : (filterclassitem.ticketClass == "Tiết kiệm" || ((filterclassitem.ticketClass == "Phổ thông đặc biệt") && filterclassitem.ticketCondition && !filterclassitem.ticketCondition.isTicketRefund && !filterclassitem.ticketCondition.isChangeSegment )) ;
              // }
              if(se.classSelected.indexOf(1) != -1){
