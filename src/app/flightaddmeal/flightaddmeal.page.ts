...
import { NavController,Platform, ModalController, ActionSheetController, PickerController, IonSlides } from '@ionic/angular';
...
constructor(public platform: Platform,public navCtrl: NavController, public modalCtrl: ModalController,public valueGlobal:ValueGlobal,
  ...
  plusQuantity(item){
    //   if(item.quantity == this._flightService.itemFlightCache.pax){
    //     this.gf.showToastWarning("Số suất ăn không được vượt quá số khách. Xin vui lòng kiểm tra lại!");
    //   }
    this.zone.run(()=>{
        item.quantity++;
    })
