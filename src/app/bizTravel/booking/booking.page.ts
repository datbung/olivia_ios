      ...
      getdatanewtoken(token, ishistory) {
        ...
        if (token) {
          ...
          var options = {
            ...
            //url: 'http://localhost:34290/api/dashboard/getmytrip?getall=true&memberid=eb0cd8f2-cb00-4293-b38e-9cf1fb6b6dc4',
            headers:
            {
              'accept': 'application/json',
              'content-type': 'application/json-patch+json',
...
//                      if(element.status != 4 &&element.status != 1 && (element.status != 2 || (element.status == 2 && timediffdays <=0) )){
    ...
    async openPickupCalendar(openDefault: boolean) {
            ...
            if (event.data) {
              ...
              //se.gf.setCacheSearchHotelInfo({checkInDate: se.bizTravelService.checkInDate, checkOutDate: se.bizTravelService.checkOutDate, adult: se.searchhotel.adult, child: se.searchhotel.child, childAge: se.searchhotel.arrchild, roomNumber: se.searchhotel.roomnumber});
            }
