...
export class TopdealfilterPage implements OnInit {
  ...
  constructor(private navCtrl: NavController, private gf: GlobalFunction,
    ...
    public topDealService: topDealService) { 
      ...
      // if(this.topDealService.objectFilter && !isNaN(this.topDealService.objectFilter.minprice) && !isNaN(this.topDealService.objectFilter.maxprice)){
      //   this.zone.run(()=>{
      //         this.priceobject = { lower: this.topDealService.objectFilter.minprice.toString(), upper: this.topDealService.objectFilter.maxprice.toString() };
      //   })
      // }else{
      //     if(this.topDealService.listData && this.topDealService.listData.length >0){
      //       this.zone.run(()=>{
      //         let maxValue = Math.max(...this.topDealService.listData.map(o => o.minPrice), 0);
      //         let minValue = Math.min(...this.topDealService.listData.map(o => o.minPrice));
          ...
          if(this.topDealService.objectFilter && this.topDealService.objectFilter.selectedRegion && this.topDealService.objectFilter.selectedRegion.length >0){
            this.selectedRegion = this.topDealService.objectFilter.selectedRegion;
              setTimeout(()=> {
                ...
                //   if(!this.gf.checkExistsIndex(this.selectedRegion,element.regionId)){
                //     this.renderDisabledItem(element.regionId);
                //   }
                // }
              },300)
          }
  ...
  renderDisabledItem(strIndex: string){
    ...
    filter(){
        ...
        // if(this.topDealService.objectFilter && (
        //   this.topDealService.objectFilter.minprice*1 != this.minvalue*1
        //   || this.topDealService.objectFilter.selectedRegion.length >0
        // ))
        // {
        //   this.topDealService.itemCountFilter.emit(1);
        // }else{
        //   this.filterItem();
        //   this.topDealService.itemCountFilter.emit(0);
        // }
