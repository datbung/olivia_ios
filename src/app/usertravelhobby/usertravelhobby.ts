...
export class UserTravelHobbyPage implements OnInit{
  ...
  public dataFilter; dataLocal = [];dataFacility=[];dataStyle=[];dataHotelType=[];dataTrademark=[];
  strLocal="";
  strFacility="";
  strStyle="";
  strHotelType="";
  ...
  constructor(public platform: Platform,public navCtrl: NavController, public zone: NgZone, public searchhotel: SearchHotel, public authService: AuthService, private http: HttpClientModule, public events:Events,
    public gf: GlobalFunction,public modalCtrl: ModalController,public storage: Storage, public alertCtrl: AlertController,public valueGlobal: ValueGlobal) {
    ...
    var se = this;
      se.storage.get('auth_token').then(auth_token => {
        ...
        request(options, function (error, response, body) {
          if (error) {
            error.page="usertravelhobby";
            error.func="GetRecommentSetting";
    ...
    //if(se.memberid){
      se.storage.get('auth_token').then(auth_token => {
        ...
        request(options, function (error, response, body) {
          if (error) {
            error.page="usertravelhobby";
            error.func="SetRecommentSetting";
            error.param =  JSON.stringify(options);
            C.writeErrorLog(error,response);
          }
